...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if data[0] == "category":
            ...
            quantity = context.user_data['quantities'].get(product.id, 1)
            if quantity < 1:
                quantity = 1
            ...
            if 'quantities' not in context.user_data:
                context.user_data['quantities'] = {}
            context.user_data['quantities'][product_id] = context.user_data['quantities'].get(product_id, 1) + 1
            ...
            product_id = int(data[1])
            if 'quantities' in context.user_data and product_id in context.user_data['quantities'] and context.user_data['quantities'][product_id] > 1:
            ...
            product_id = int(data[1])
            quantity = context.user_data['quantities'].get(product_id, 1)
            if quantity > 0:
                carts = context.user_data.get("carts", {})
                carts[product_id] = carts.get(product_id, 0) + quantity
                context.user_data["carts"] = carts
                context.user_data['quantities'][product_id] = 1  # Reset quantity after adding to cart
            await query.message.delete()
            ...
            new_markup = InlineKeyboardMarkup([
                [InlineKeyboardButton(BTN_ORDER[lang_id], callback_data="order"),
                 InlineKeyboardButton(CLEAN[lang_id], callback_data="cartclear")],
                [InlineKeyboardButton(BACK[lang_id], callback_data="cartback")]
            ])
            ...
            await context.bot.send_message(
                ...
                text=CHOOSE_PAYMENT_TYPE[lang_id],
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton(CASH[lang_id], callback_data="orderpayment_1"),
                     InlineKeyboardButton("Payme", callback_data="orderpayment_2"),
                     InlineKeyboardButton("Click", callback_data="orderpayment_3")]
                ])
            )
        elif data[0] == "orderpayment":
            context.user_data['paymenttype'] = int(data[1])
            if not user.phone_number:
                logger.info(f"Phone number seems does not valid: {user.phone_number}")
